  ...
  return (
    <section className="panel p-3 sm:p-4">
        ...
        <div className="mt-3 rounded-3xl border border-white/10 bg-slate-950/48 px-3 py-3 text-sm font-bold leading-6 text-slate-200">
          ...
          <div className="grid max-h-28 gap-2 overflow-y-auto pr-1">
            {snapshot.notes.slice(-3).reverse().map((note) => (
              <div key={note} className="break-words rounded-2xl border border-white/8 bg-white/5 px-3 py-2">{note}</div>
            ))}
          </div>
        </div>
      ) : null}
      ...
      {!isMyTurn ? (
        ...
        <div className="mt-2 flex items-center gap-2 rounded-2xl border border-white/10 bg-slate-950/48 px-4 py-3 text-sm font-bold text-slate-300">
          <AlertTriangle size={15} className="text-amber-200" /> 행동을 마쳤다. 턴을 넘겨라.
        </div>
      ) : null}
    </section>
  );
