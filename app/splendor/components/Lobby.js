...
export default function Lobby({
  ...
  const requireUser = () => {
    ...
    pushNotice('error', '잠시 후 다시 시도해 주세요.');
    return false;
  };
  ...
  const copyInviteLink = async () => {
    ...
    try {
      ...
      pushNotice('error', '초대 링크를 복사하지 못했다.');
    }
  };
  ...
  const handleCreate = async () => {
    ...
    if (!nickname) return pushNotice('error', '이름을 입력해 주세요.');
    ...
    try {
      ...
      if (!createdCode) throw new Error('방 코드를 만들지 못했다.');
      setRoomCode(createdCode);
      setIsInviteMode(false);
      pushNotice('success', `${createdCode} 방을 만들었다.`);
    } catch (error) {
      pushNotice('error', error?.message || '방을 만들지 못했다.');
    }
  };
  ...
  const handleJoin = async () => {
    ...
    if (!nickname) return pushNotice('error', '이름을 입력해 주세요.');
    if (code.length !== 4) return pushNotice('error', '방 코드는 4자리다.');
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        if (!lobbyIds.includes(user.uid) && lobbyIds.length >= ROOM_MAX_PLAYERS) throw new Error('방이 가득 찼다.');
      ...
      setRoomCode(code);
      setIsInviteMode(false);
      pushNotice('success', `${code} 방에 들어갔다.`);
    } catch (error) {
      pushNotice('error', error?.message || '입장하지 못했다.');
    }
  };

  const handleAddBot = async () => {
    if (!isHost) return;
    if (lobbyPlayers.length >= ROOM_MAX_PLAYERS) return pushNotice('error', '방이 가득 찼다.');
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        if (room.status !== 'lobby') throw new Error('이미 시작된 방이다.');
        const lobbyIds = Array.isArray(room.lobbyPlayerIds) ? [...room.lobbyPlayerIds] : [];
        if (lobbyIds.length >= ROOM_MAX_PLAYERS) throw new Error('방이 가득 찼다.');
        lobbyIds.push(botId);
        tx.set(roomRef, { lobbyPlayerIds: lobbyIds, updatedAt: serverTimestamp(), presenceAt: serverTimestamp() }, { merge: true });
        tx.set(playerRef, { ...createPublicPlayerPayload(botName, true), name: botName, isBot: true, online: true, lastSeenAt: serverTimestamp() });
        tx.set(playerPrivateRef, createPrivatePlayerPayload());
      });
      pushNotice('success', '기록관을 추가했다.');
    } catch (error) {
      pushNotice('error', error?.message || '기록관을 추가하지 못했다.');
    }
  };
  ...
  const handleRemoveBot = async (botId) => {
    ...
    try {
      ...
      pushNotice('success', '기록관을 제거했다.');
    } catch (error) {
      pushNotice('error', error?.message || '제거하지 못했다.');
    }
  };
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        if (!live.length) throw new Error('참가 인원이 없다.');
      ...
      pushNotice('success', '수사를 시작했다.');
    } catch (error) {
      pushNotice('error', error?.message || '수사를 시작하지 못했다.');
    }
  };
      ...
      <div className="mx-auto flex w-full max-w-[560px] flex-col px-4 pb-[calc(2rem+var(--safe-bottom))] pt-[calc(1rem+var(--safe-top))]">
        <section className="panel p-4 sm:p-5">
          <div className="border-b border-white/10 pb-5">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <h1 className="mt-1 break-words text-[28px] font-black leading-tight text-[#f7efe3]">{CASE_TITLE}</h1>
                <p className="mt-2 text-sm font-bold leading-6 text-[#dfcfba]">전시장 안에서 시신이 발견됐다. 왕관도 사라졌다.</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 pt-4">
            ...
            {isInviteMode ? (
              <div className="rounded-2xl border border-[#6b8196]/20 bg-[#223142]/18 px-4 py-3 text-sm font-black text-[#e3edf7]">초대 링크로 접속했다.</div>
            ) : null}

            {roomData?.status === 'lobby' ? (
              <div className="rounded-[24px] border border-white/10 bg-black/10 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="text-sm font-black text-[#f7efe3]">대기 인원 {lobbyPlayers.length}/{ROOM_MAX_PLAYERS}</div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {isHost ? (
                      <button type="button" onClick={handleAddBot} className="tap-feedback min-h-11 rounded-2xl border border-white/10 bg-black/10 px-3 py-2 text-sm font-black text-[#f2e7d3]">
                        <span className="inline-flex items-center gap-2 whitespace-normal"><Bot size={14} /> 기록관 추가</span>
                      </button>
                    ) : null}
                  </div>
                </div>
                  ...
                  {lobbyPlayers.length ? lobbyPlayers.map((player) => {
                    ...
                    return (
                      <div key={player.id} className="panel-soft flex items-center justify-between gap-3 px-4 py-3">
                        <div className="min-w-0">
                          <div className="break-words text-sm font-black text-[#f7efe3]">{getDisplayName(player)}</div>
                          <div className="mt-1 flex flex-wrap gap-2 text-[11px] font-black text-[#cdbda8]">
                            ...
                            {player.isBot ? <span className="rounded-full border border-[#6b8196]/25 bg-[#223142]/18 px-2 py-0.5 text-[#e3edf7]">기록관</span> : null}
                        ...
                        {isHost && player.isBot ? (
                          <button type="button" onClick={() => handleRemoveBot(player.id)} className="tap-feedback rounded-2xl border border-white/10 bg-black/10 p-2 text-[#d5c7b4]" aria-label="기록관 제거">
                            <X size={15} />
                          </button>
                        ) : null}
                      </div>
                    ...
                    <div className="panel-soft px-4 py-4 text-sm font-black text-slate-400">참가 인원이 없다.</div>
                  )}
                </div>
