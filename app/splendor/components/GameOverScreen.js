      ...
      <div className="mx-auto min-h-screen w-full max-w-[480px] px-3 py-4">
        <section className="panel p-4">
          <div className="border-b border-white/10 pb-5 text-center">
            ...
            <div className="mt-3 text-[11px] font-black tracking-[0.22em] text-[#c7ae84]">사건 정리</div>
            <div className="mt-2 text-2xl font-black text-[#f7efe3]">{reveal?.headline || '결과'}</div>
            <div className="mt-3 break-words text-sm font-bold leading-6 text-[#e6d8c2]">{reveal?.summary || '기록이 끝났다.'}</div>
          ...
          {reveal?.endingLines?.length ? (
            <div className="grid gap-2 border-b border-white/10 py-4">
              {reveal.endingLines.map((line) => (
                <div key={line} className="rounded-2xl border border-white/10 bg-black/10 px-4 py-3 text-sm font-bold text-[#ecdfca] break-words">
                  {line}
                </div>
              ))}
            </div>
                ...
                {report.notebook?.notes?.length ? (
                  <div className="mt-3 grid max-h-32 gap-2 overflow-y-auto pr-1 text-sm font-bold text-[#ecdfca]">
                    {report.notebook.notes.slice(-3).reverse().map((note) => (
                      <div key={note} className="break-words rounded-2xl border border-white/8 bg-[#17120f]/70 px-3 py-2">
                        {note}
                      </div>
                    ))}
                  </div>
