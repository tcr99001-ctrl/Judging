...
function ChoiceGroup({ title, items, activeId, onPick, formatter = (item) => item.label || item.name }) {
  return (
    <div className="rounded-[20px] border border-white/10 bg-slate-950/44 px-4 py-4">
        ...
        {items.map((item) => (
          <button
            ...
            className={[
              ...
              activeId === item.id ? 'border-rose-300/20 bg-rose-500/10 text-rose-50' : 'border-white/10 bg-slate-900/46 text-slate-200',
            ].join(' ')}
          >
            {formatter(item)}
          </button>
  ...
  return (
    <div className="modal-layer">
      ...
      <div className="panel modal-panel max-w-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="min-h-0 overflow-y-auto px-4 py-4">
          <div className="grid gap-4">
            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-[18px] border border-white/10 bg-slate-900/46 px-3 py-3 text-center">
        ...
        <div className="grid shrink-0 gap-2 border-t border-white/10 px-4 py-4 sm:grid-cols-[1fr_auto]">
          ...
          <button
            ...
            disabled={!enabled}
            className="tap-feedback min-h-12 rounded-2xl border border-rose-300/20 bg-rose-500/10 px-5 py-3 text-sm font-black text-rose-50 disabled:border-slate-800 disabled:bg-slate-950 disabled:text-slate-500"
          >
