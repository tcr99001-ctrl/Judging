        ...
        <div className="min-h-0 overflow-y-auto px-4 py-4">
          <div className="grid gap-3">
            {privateClues.length ? privateClues.map((card) => {
              ...
              return (
                <div key={card.id} className="rounded-[22px] border border-white/10 bg-slate-950/42 p-3">
                  <div className="grid gap-3 sm:grid-cols-[140px_1fr_auto] sm:items-center">
                    <CardFace card={card} size="mini" />
                    <div>
                      <div className="break-words text-sm font-black text-white">{card.title}</div>
                      <div className="mt-2 break-words text-sm font-bold leading-6 text-slate-300">{card.summary}</div>
                    ...
                    <button
                      ...
                      className="tap-feedback min-h-12 rounded-2xl border border-amber-300/20 bg-amber-500/10 px-4 py-3 text-sm font-black text-amber-50 disabled:border-slate-800 disabled:bg-slate-950 disabled:text-slate-500"
                    >
